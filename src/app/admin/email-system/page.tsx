...
function EmailSystemPage() {
    ...
    const fetchEvents = async () => {
        try {
            // Reusing WhatsApp events API as it just returns events
    ...
    const fetchLogs = useCallback(async (eventId: string) => {
        setDataLoading(true);
        try {
            const response = await fetch(`/api/email/logs?event_id=${eventId}`);
            const data = await response.json();
            if (response.ok) {
    ...
    const fetchHistory = async (guest: GuestLog) => {
        ...
        try {
            const response = await fetch(`/api/email/history?guest_id=${guest.id}&event_id=${selectedEventId}`);
            const data = await response.json();
            if (response.ok) {
                setHistoryLogs(data);
            }
    ...
    const handleSendClick = (guest: GuestLog) => {
        if (!guest.email) {
            toast.warning("This guest doesn't have an email address.");
            return;
        }
    ...
    }, [selectedEventId]); // Removed fetchLogs from dependencies to avoid infinite loop
        ...
        return (
            <div className="flex items-center justify-center py-40">
                <div className="flex flex-col items-center gap-4 text-center">
                    ...
                    <p className="text-xs font-black tracking-[0.3em] uppercase text-zinc-400 animate-pulse">Initializing Email System...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-[1.75rem] md:text-4xl font-black text-zinc-900 dark:text-zinc-50 tracking-tight">Email System</h1>
                    </div>
                    <p className="text-zinc-500 dark:text-zinc-400 font-medium">Monitor email delivery and communication history</p>
                </div>
                                    ...
                                    <tr key={log.id} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-all group">
                                        <td className="sticky left-0 z-10 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md px-4 md:px-8 py-5 shadow-[2px_0_10px_rgba(0,0,0,0.05)]">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="flex flex-col min-w-0 max-w-[150px] md:max-w-none">
                                                    <span className="text-sm font-bold text-zinc-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">{log.name}</span>
                                                    <span className="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono truncate">{log.email || 'No Email Address'}</span>
                                                </div>
                                            </div>
                                        </td>
                            ...
                            <div className="p-6 md:p-10 border-b border-zinc-100 dark:border-white/5 flex items-start justify-between bg-zinc-50/50 dark:bg-zinc-900/50">
                                <div className="flex items-center gap-4 md:gap-6">
                                    ...
                                    <div className="space-y-0.5 md:space-y-1">
                                        <h2 className="text-xl md:text-3xl font-black text-zinc-900 dark:text-white tracking-tight leading-tight">{historyGuest.name}</h2>
                                        <p className="text-zinc-500 font-bold font-mono text-[10px] md:text-sm tracking-widest uppercase truncate max-w-[200px] md:max-w-none">{historyGuest.email}</p>
                                    </div>
                                </div>
                                <Button
                                    variant="ghost"
                            ...
                            <div className="p-6 md:p-10 overflow-y-auto flex-1 bg-white dark:bg-zinc-900">
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Email History Trail</h3>
                                    ...
                                    {historyLoading ? (
                                        <div className="flex flex-col items-center justify-center py-20 gap-4">
                                            ...
                                            <span className="text-xs font-bold tracking-widest uppercase text-zinc-400">Loading History...</span>
                                        </div>
                                    ) : historyLogs.length === 0 ? (
                                        <div className="text-center py-20 text-zinc-400 font-bold italic">No email data found.</div>
                                    ) : (
                        ...
                        >
                            <div className="p-8 border-b border-zinc-100 dark:border-white/5 flex items-start justify-between bg-zinc-50/50 dark:bg-zinc-900/50">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="space-y-1">
                                        <h2 className="text-2xl font-black text-zinc-900 dark:text-white tracking-tight">Select Email Template</h2>
                            ...
                            <div className="p-8 space-y-6">
                                <div className="grid grid-cols-1 gap-3">
                                    {[
                                        ...
                                        { id: 'Alert', label: 'Service Alert', icon: CheckCircle2, color: 'emerald' },
                                    ].map((template) => {
...
export default withRoleAuth(EmailSystemPage, "admin");
